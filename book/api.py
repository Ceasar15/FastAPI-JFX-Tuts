...
def create_book(request: schemas.BookSchema, db: Session = Depends(get_db)):
...
def get_specific_book(book_id: str, db: Session = Depends(get_db)):
    return book.get_specific_book(book_id, db)
